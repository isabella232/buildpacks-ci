...
class DotnetFrameworkExtractor
  ...
  def extract_runtime
    ...
    version = extract_framework_dep(@sdk_dir, runtime_tar, %w(Microsoft.NETCore.App))

    create_dependency(runtime_tar, version, @source_input, build_output, 'dotnet-runtime')
  end

  def extract_aspnetcore
    build_output = BuildOutput.new('dotnet-aspnetcore')
    aspcorenet_tar = File.join(@base_dir, "dotnet-aspnetcore.tar.xz")

    version = extract_framework_dep(@sdk_dir, aspcorenet_tar, %w(Microsoft.AspNetCore.App Microsoft.AspNetCore.All))
  ...
  def extract_framework_dep(sdk_dir, tar, package_names)
    Dir.chdir(sdk_dir) do
      ...
      FileUtils.rm_rf(paths) or raise "Failed to remove #{package_names.join(', ')} from sdk"
      return version
    end
  end
