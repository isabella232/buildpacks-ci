...
module DependencyBuild
  ...
  def build_dotnet_runtime(source_input)
    prune_dotnet_files(source_input, ["./sdk", "./dotnet", "./shared/Microsoft.AspNetCore.App/"])
  end

  def build_dotnet_aspnetcore(source_input)
    prune_dotnet_files(source_input, ["./sdk", "./dotnet", "./shared/Microsoft.NETCore.App/"])
  end

  def prune_dotnet_files(source_input, files_to_exclude)
    source_file = Dir.glob('sources/*.tar.gz').first
    ...
    `tar -Jcf #{adjusted_file} #{exclude_list} @#{source_file}`
    adjusted_file
  end
